...
# --- START: Critical Configuration Section ---
# This section MUST run before any of your other project files are imported.

# Load environment variables from .env file FIRST.
load_dotenv()

# --- START: TEMPORARY DEBUG CODE ---
# This code will print information to your TERMINAL.
print("="*30)
print("RUNNING DIAGNOSTIC CHECK...")
# 1. We check WHERE the program is running from.
print(f"Current Working Directory: {os.getcwd()}") 
# 2. We check IF the .env file can be found and read by the library.
from dotenv import find_dotenv
dotenv_path = find_dotenv()
if dotenv_path:
    print(f".env file found at: {dotenv_path}")
else:
    print(".env file NOT FOUND in this directory or parent directories.")
# 3. We check if the KEY is inside the file that was loaded.
api_key_debug = os.getenv("GOOGLE_API_KEY")
if api_key_debug:
    # We only print the last 4 characters for security.
    print(f"GOOGLE_API_KEY Found: Yes (ends with '...{api_key_debug[-4:]}')") 
else:
    print("GOOGLE_API_KEY Found: NO")
print("="*30)
# --- END: TEMPORARY DEBUG CODE ---

# Immediately configure the genai library with the API key.
# This makes the configuration available globally for the genai module.
try:
    api_key = os.getenv("GOOGLE_API_KEY")
    if not api_key:
        # If the key is not found, stop the app with a clear error.
        # This prevents the downstream "No API_KEY" error and shows the root cause.
        st.error("🔴 GOOGLE_API_KEY not found in your environment. Please set it in your .env file or system variables.")
        st.stop()
...
# Initialize the model once and use it throughout the app.
# --- THIS IS THE UPDATED LINE ---
model = genai.GenerativeModel("gemini-2.5-flash")
# --- END: Critical Configuration Section ---


# Now, import your orchestrator. Since genai is already configured,
# this import *should* inherit the configured state.
try:
    from orchestrator import SimpleAgenticOrchestrator
except ImportError:
    st.error("Could not import SimpleAgenticOrchestrator. Please ensure orchestrator.py is in the correct path.")
    st.stop()
...
# Main content
st.markdown('<h1 class="main-header">🌾 FarmGPT</h1>', unsafe_allow_html=True)
...
<div style="text-align: center; color: #666; padding: 1rem;">
    <p>🌾 FarmGPT - Grow Smarter , Farm Better .</p>
