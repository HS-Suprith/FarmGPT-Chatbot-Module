...
class SimpleAgenticOrchestrator:
    ...
    # In simple_orchestrator.py, replace the whole function with this one.
    ...
    def call_agent(self, agent_name, parameters):
        ...
        try:
            ...
            if agent_name == 'crop_advisor':
                ...
                location = parameters.get('location') or 'Channarayapatna' # Using your location as default
                return agent_function(soil, location)
                
            elif agent_name == 'market_broker':
                crop = parameters.get('crop') or 'wheat'
                location = parameters.get('location') or 'Karnataka'
                quantity = parameters.get('quantity')
                return agent_function(crop, location, quantity)
                
            elif agent_name == 'disease_detector':
                # This is the corrected logic
                image_file = parameters.get('image_file')
                if image_file:
                    # Actually call the analyze_crop_image function
                    return agent_function(image_file)
                else:
                    return "To detect a disease, please upload an image of the crop."
                
            elif agent_name == 'alert_system':
                crop = parameters.get('crop') or 'wheat'
                location = parameters.get('location') or 'Karnataka'
                return agent_function(crop, location)

            else:
                # Fallback for any other agent that might be added
                return agent_function(parameters)
