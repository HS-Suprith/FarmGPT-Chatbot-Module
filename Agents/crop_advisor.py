...
def get_weather(city):
    if not WEATHER_API_KEY:
        print("❌ WEATHER_API_KEY is missing. Please add it to your .env file.")
    ...
    try:
        ...
        weather_summary = (
            f"Date: {date}, {condition}, "
            f"Max Temp: {max_temp}°C, Min Temp: {min_temp}°C, "
            f"Rain Chance: {rain_chance}%, Rain Expected: {rain_expected}%"
        )

        print(f"🌤️ Weather in {city} on {date}: {condition}, "
              f"Max Temp: {max_temp}°C, Min Temp: {min_temp}°C, "
              f"Rain Chance: {rain_chance}%, Rain Expected: {rain_expected}%")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            advice, weather = get_crop_advice(soil, city)
            print("🤖 Advisor:\n")
            print(advice)
        except Exception as e:
            print("❌ Error:", e)
